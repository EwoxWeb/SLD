...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_SESSION['nom_compte'])) {
    ...
    if ($_SESSION['nom_compte'] == 'Ewox_') {
        ...
        $stmt_generation = $conn->prepare("SELECT generation FROM pkm_form WHERE id_pkm = :id_pkm");
        ...
        $stmt = $conn->prepare("UPDATE pkm_form SET methode = :methode, compteur = :compteur WHERE id_pkm = :id_pkm");
